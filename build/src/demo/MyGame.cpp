...
MyGame::MyGame() : AbstractGame(), score(0), lives(3), numKeys(5), gameWon(false), box(5, 5, 30, 30) {
	...
	// creating objects to hold data about the player and the enemy(s) 
	std::cout << "creating player holder" << std::endl;
	Player player1 = Player(box.x, box.y);
	// defining the enemy
	std::cout << "creating enemy" << std::endl;
	Enemy *enemy1 =  &Enemy(getRandom(0, 750), getRandom(0, 550), 2.5, 0.1,false);
	//std::shared_ptr<Enemy> t = std::make_shared<Enemy>();
